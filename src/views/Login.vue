        ...
        <div class="relative hidden w-full items-center justify-center lg:flex lg:w-1/2 illustration-bg"
            ...
            <div class="z-10 px-12 text-center text-white">
                ...
                <p class="mx-auto max-w-md text-lg font-medium leading-relaxed opacity-90">
                    تمكين الطلاب والأكاديميين من الوصول إلى تجربة تعليمية حديثة ومبتكرة عبر أحدث تقنيات التعليم
                    الإلكتروني.
                </p>
            ...
            <main class="flex-1 flex flex-col justify-center px-8 lg:px-24 py-12">
                <div class="w-full max-w-md mx-auto">
                    ...
                    <form class="space-y-6" @submit.prevent="handleLogin">
                        <div>
                            <label class="block text-sm font-bold text-text-main mb-2 text-right" for="username">اسم
                                المستخدم أو البريد الجامعي</label>
                            <div class="relative">
                                <input v-model="form.username"
                                    ...
                                    id="username" placeholder="example@university.edu" type="text" />
                                <div
                                    class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none text-text-muted">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <div class="absolute inset-y-0 left-0 flex items-center pl-4 cursor-pointer text-text-muted hover:text-primary"
                        ...
                        <div class="flex items-center gap-2 text-right">
                            <input v-model="form.remember"
                                class="w-4 h-4 text-primary border-border-base rounded focus:ring-primary bg-bg-surface"
                    ...
                    <div class="mt-10 text-center">
                        <p class="text-sm text-text-muted">
                            ليس لديك حساب؟
                            <RouterLink class="text-primary font-bold hover:underline" to="/register">انشئ حسابك الأن
                            </RouterLink>
                        </p>
...
const handleLogin = async () => {
    ...
    if (!form.username.trim()) {
        showToast('خطأ', 'يرجى إدخال البريد الإلكتروني أو اسم المستخدم.', 'error')
        return
    }
    if (!form.password) {
    ...
    try {
        ...
        console.log('[Login] token in localStorage:', localStorage.getItem('auth_token'))

        if (authStore.isAuthenticated) {
            showToast('تسجيل الدخول', 'تم تسجيل الدخول بنجاح، جاري تحويلك...')

            const role = authStore.userRole || form.role
            const redirectTo = router.currentRoute.value.query.redirect
            const dashboard = redirectTo || (role === 'instructor' ? '/instructor/dashboard' : '/student/dashboard')

            console.log('[Login] Redirecting to:', dashboard)
            router.replace(dashboard)
        } else {
            console.warn('[Login] ⚠️ Login API succeeded but token was not stored!')
        ...
        showToast('خطأ في تسجيل الدخول', msg, 'error')
    } finally {
