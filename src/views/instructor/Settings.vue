<template>
    <div class="flex flex-col h-full bg-bg-base text-text-main font-sans transition-colors duration-300 relative">
        ...
        <header
            class="sticky top-0 z-50 w-full bg-bg-surface border-b border-border-base px-4 md:px-10 py-3 transition-colors duration-300 shrink-0">
            <div class="max-w-[1440px] mx-auto flex items-center justify-between whitespace-nowrap">
                <div class="flex items-center gap-8">
                    ...
                    <div class="md:hidden flex items-center gap-3">
                        <div
                            class="size-10 bg-primary rounded-xl flex items-center justify-center text-white shadow-sm">
                            <span class="material-symbols-outlined text-2xl">school</span>
                        </div>
                        <h2 class="text-text-main text-xl font-black leading-tight tracking-tight font-display">Smart
                            Learn</h2>
                    </div>

                    <nav class="hidden md:flex items-center gap-8">
                        <router-link to="/instructor/dashboard"
                            class="text-text-main text-sm font-medium hover:text-primary transition-colors">الرئيسية</router-link>
                        <router-link to="/instructor/courses"
                            class="text-text-muted text-sm font-medium hover:text-primary transition-colors">المقررات</router-link>
                    </nav>
                </div>
                <div class="flex items-center gap-4">
                    <div class="flex gap-2">
                        <button
                            class="flex items-center justify-center rounded-lg size-10 bg-bg-base text-text-main hover:bg-bg-base-hover transition-colors">
        ...
        <div class="max-w-[1440px] mx-auto w-full p-4 md:p-10 flex flex-col gap-6 flex-1 overflow-y-auto">
                ...
                <div class="flex flex-wrap justify-between items-end gap-4">
                    ...
                    <div class="flex gap-3">
                        <button @click="discardChanges"
                            class="px-6 py-2.5 bg-bg-base text-text-main font-bold rounded-lg text-sm transition-all hover:bg-bg-base-hover cursor-pointer border border-transparent hover:border-border-base">تجاهل
                            التغييرات</button>
                        <button @click="saveChanges" :disabled="isSaving"
                            class="px-6 py-2.5 bg-primary text-white font-bold rounded-lg text-sm shadow-lg shadow-primary/25 transition-all hover:brightness-110 cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed flex items-center gap-2">
                            <span v-if="isSaving"
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                <div class="xl:col-span-2 flex flex-col gap-6">
                    <section
                        class="bg-bg-surface rounded-xl shadow-sm border border-border-base overflow-hidden transition-colors">
                            ...
                            <div class="flex flex-col gap-4">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input class="sr-only peer" type="checkbox" v-model="isDarkMode"
                                            @change="toggleDarkMode" />
                                        <div
                                            class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-[-100%] rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:right-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary transition-colors">
                                        </div>
                                    </label>
                ...
                <div class="xl:col-span-1">
                    <section
                        class="bg-bg-surface rounded-xl shadow-sm border border-border-base p-8 flex flex-col items-center justify-center text-center transition-colors">
                        <div class="relative group cursor-pointer mb-6" @click="triggerFileUpload">
                            <div
                                class="size-40 rounded-full border-4 border-primary/10 overflow-hidden bg-bg-base relative">
                                ...
                                <Transition name="fade">
                                    <div v-if="isUploading"
                                        ...
                                        <span
                                            class="size-10 border-4 border-white/30 border-t-white rounded-full animate-spin"></span>
                                    </div>
                                </Transition>
                            </div>
                            <div
                            ...
                            <button
                                class="absolute bottom-1 right-1 size-10 bg-primary text-white rounded-full flex items-center justify-center shadow-lg border-4 border-white dark:border-gray-900 cursor-pointer hover:bg-primary-hover transition-colors">
                        ...
                        <button @click="triggerFileUpload"
                            class="w-full flex items-center justify-center gap-2 px-6 py-2.5 border-2 border-primary/20 text-primary font-bold rounded-lg text-sm hover:bg-primary/5 transition-colors cursor-pointer">
                ...
                <div class="p-6">
                    <form class="grid grid-cols-1 md:grid-cols-3 gap-6" @submit.prevent>
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="relative">
                                <input v-model="passwords.current"
                                    class="w-full px-4 py-2.5 rounded-lg border border-border-base bg-transparent focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-text-main"
                                    placeholder="••••••••" :type="showPassword.current ? 'text' : 'password'" />
                                <button type="button" @click="showPassword.current = !showPassword.current"
                                    class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-text-muted text-xl cursor-pointer hover:text-primary transition-colors">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="relative">
                                <input v-model="passwords.new"
                                    class="w-full px-4 py-2.5 rounded-lg border border-border-base bg-transparent focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-text-main"
                                    placeholder="أدخل كلمة مرور قوية" :type="showPassword.new ? 'text' : 'password'" />
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="relative">
                                <input v-model="passwords.confirm"
                                    class="w-full px-4 py-2.5 rounded-lg border border-border-base bg-transparent focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-text-main"
                    ...
                    <div class="flex justify-end mt-6 border-t border-border-base pt-6">
                        <button type="submit" @click="updatePassword" :disabled="isUpdatingPassword || !isPasswordValid"
                            class="px-6 py-2 bg-primary text-white font-bold rounded-lg text-sm shadow-md transition-all hover:brightness-110 cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed flex items-center gap-2">
                    ...
                    <div
                        class="mt-8 p-4 bg-primary/5 dark:bg-primary/10 rounded-lg border border-primary/10 flex gap-3">
            ...
            <section
                class="bg-bg-surface rounded-xl shadow-sm border border-border-base p-6 flex items-center justify-between transition-colors">
                <div class="flex items-center gap-4">
                    <div
                ...
                <div class="flex gap-2">
                    <span
                        class="px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-xs font-bold rounded-full">نشط
                        - معتمد</span>
                </div>
            </section>
            ...
            <!-- Delete Account -->
            <section
                class="bg-bg-surface rounded-xl shadow-sm border border-red-200 dark:border-red-900 overflow-hidden transition-colors">
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-text-muted mt-1">سيتم حذف جميع بياناتك ودوراتك ولا يمكن التراجع عن
                                هذا الإجراء.</p>
                        </div>
        ...
        <ConfirmDialog :visible="showDeleteDialog" title="حذف الحساب نهائياً"
            message="سيتم حذف جميع بياناتك بما في ذلك الدورات والمحاضرات والطلاب المسجلين. هذا الإجراء لا يمكن التراجع عنه."
