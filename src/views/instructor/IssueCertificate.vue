    ...
    <div class="flex-1 flex flex-col min-w-0 transition-all duration-300">
      <header
        ...
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-3">
            <div
              class="bg-primary text-white rounded-xl p-1.5 flex items-center justify-center shadow-md shadow-primary/20">
        ...
        <div class="flex items-center gap-6">
          <button @click="saveDraft" :disabled="isSaving"
            class="bg-primary text-white px-6 py-2 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-primary/25 hover:opacity-90 transition-opacity cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed">
            <span v-if="isSaving"
      ...
      <div class="p-8 max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8 w-full">
        <div class="lg:col-span-3 space-y-6">
          <div class="bg-bg-surface p-6 rounded-2xl border border-border-base shadow-sm transition-colors duration-300">
            <div class="flex items-center justify-between relative px-4">
              ...
              <div class="relative z-10 flex flex-col items-center gap-2">
                <div
                  class="size-10 rounded-full bg-primary text-white flex items-center justify-center font-bold ring-4 ring-primary/20">
                  4</div>
          ...
          <div class="bg-bg-surface border border-border-base rounded-3xl p-8 transition-colors duration-300">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-2 gap-8">
              <div class="space-y-6">
                <div
                  class="border-2 border-dashed border-primary/30 rounded-2xl p-8 text-center bg-primary/5 hover:bg-primary/10 transition-colors cursor-pointer">
                  <div
                    class="size-16 rounded-full bg-bg-surface flex items-center justify-center mx-auto mb-4 shadow-sm">
                ...
                <div class="space-y-4">
                  <div
                    class="bg-bg-base/50 p-4 rounded-xl border border-border-base flex items-center justify-between transition-colors duration-300">
                    <div class="flex items-center gap-3">
                      <div
                        class="size-10 bg-green-50 dark:bg-green-900/20 rounded-lg flex items-center justify-center text-green-600">
                    ...
                    <input :checked="true"
                      class="w-5 h-5 rounded text-primary border-gray-300 focus:ring-primary bg-bg-base border-border-base"
                      type="checkbox" />
                  </div>
                  <div
                    class="bg-bg-base/50 p-4 rounded-xl border border-border-base flex items-center justify-between transition-colors duration-300">
                    <div class="flex items-center gap-3">
                      <div
                        class="size-10 bg-blue-50 dark:bg-blue-900/20 rounded-lg flex items-center justify-center text-primary">
              ...
              <div
                class="bg-bg-base/50 rounded-2xl p-4 flex flex-col items-center justify-center min-h-[300px] border border-border-base transition-colors duration-300">
                <div
                  class="w-full aspect-[1.414/1] bg-white rounded shadow-lg p-8 relative overflow-hidden flex flex-col items-center text-center">
                  ...
                  <p class="text-[10px] text-gray-500 max-w-[200px] leading-relaxed">لاجتيازه بنجاح متطلبات دورة "مقدمة
                    في الذكاء الاصطناعي" المقامة عبر المنصة</p>
            ...
            <div class="mt-12 flex items-center justify-between pt-8 border-t border-border-base">
              <button
                class="px-8 py-3 rounded-xl border border-border-base font-bold hover:bg-bg-base transition-colors text-text-main cursor-pointer">السابق</button>
              <button @click="publishCertificate" :disabled="isPublishing"
                class="bg-primary text-white px-10 py-3 rounded-xl font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] transition-transform flex items-center gap-2 cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed">
        ...
        <div class="lg:col-span-1">
          <div
            class="bg-bg-surface border border-border-base rounded-2xl overflow-hidden sticky top-24 transition-colors duration-300">
            ...
            <div class="p-4 space-y-4">
              ...
              <button
                class="w-full py-2 border-2 border-dashed border-border-base rounded-lg text-xs font-bold text-text-muted hover:bg-bg-base transition-colors cursor-pointer">
                + إضافة محاضرة جديدة
              </button>
            </div>
...
// ── Save Draft (placeholder — API doesn't have a separate draft endpoint) ──
...
const publishCertificate = async () => {
  ...
  try {
    await coursesApi.issueCertificate(courseId.value);
    showToast('تم الإصدار', 'تم إصدار الشهادات للطلاب بنجاح');
  } catch (error) {
