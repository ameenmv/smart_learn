<template>
    <div class="flex min-h-screen flex-col lg:flex-row bg-bg-base transition-colors duration-300">
        ...
        <div class="relative hidden w-full items-center justify-center lg:flex lg:w-1/2 illustration-bg"
            data-alt="students collaborating in a modern university library setting">
            <div class="z-10 px-12 text-center text-white">
                    ...
                    <div class="flex items-center gap-4 flex-row-reverse">
                        <div class="rounded-2xl bg-white/20 p-3 backdrop-blur-md">
                            <span class="material-symbols-outlined text-4xl text-white"
                                style="font-variation-settings: 'FILL' 1, 'wght' 400, 'GRAD' 0, 'opsz' 48">school</span>
                        </div>
                ...
                <p class="mx-auto max-w-md text-lg font-medium leading-relaxed opacity-90">
                    انضم إلى آلاف الطلاب والمحاضرين في بيئة تعليمية متطورة مصممة خصيصاً لمستقبل أكاديمي مشرق عبر منصة
                    Smart Learn.
                </p>
            ...
            <main class="flex flex-1 flex-col justify-center px-6 pb-12 lg:px-24">
                <div class="mx-auto w-full max-w-[480px]">
                    ...
                    <div class="mb-8">
                        ...
                        <div
                            class="flex h-12 w-full items-center justify-center rounded-xl bg-bg-base p-1 border border-border-base">
                            <label
                    ...
                    <form class="space-y-5" @submit.prevent="handleRegister">
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <span
                                    class="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-text-muted">person</span>
                                <input v-model="form.fullName"
                                    class="w-full rounded-xl border bg-bg-surface py-3.5 pr-11 pl-4 text-sm outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 text-text-main"
                                    :class="hasError('name') ? 'border-red-500' : 'border-border-base'"
                                    placeholder="أدخل اسمك الثلاثي" type="text" />
                            </div>
                            <span v-if="getError('name')" class="text-xs text-red-500 mt-1 block">{{ getError('name')
                                }}</span>
                        </div>
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <span
                                    class="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-text-muted">mail</span>
                                <input v-model="form.email"
                                    class="w-full rounded-xl border bg-bg-surface py-3.5 pr-11 pl-4 text-sm outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 text-text-main"
                                    :class="hasError('email') ? 'border-red-500' : 'border-border-base'"
                                    placeholder="example@university.edu" type="email" />
                            </div>
                        ...
                        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <span
                                        class="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-text-muted">badge</span>
                                    <input v-model="form.studentId"
                                        class="w-full rounded-xl border bg-bg-surface py-3.5 pr-11 pl-4 text-sm outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 text-text-main"
                                        :class="hasError('university_id') ? 'border-red-500' : 'border-border-base'"
                                        placeholder="ID-000000" type="text" />
                                </div>
                                <span v-if="getError('university_id')" class="text-xs text-red-500 mt-1 block">{{
                                    getError('university_id') }}</span>
                            </div>
                            <div class="space-y-2">
                                <label class="text-sm font-semibold text-text-main">كلمة المرور</label>
                                <div class="relative">
                                    <span
                                        class="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-text-muted">lock</span>
                                    <input v-model="form.password" :type="showPassword ? 'text' : 'password'"
                                        class="w-full rounded-xl border bg-bg-surface py-3.5 pr-11 pl-12 text-sm outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 text-text-main"
                                        :class="hasError('password') ? 'border-red-500' : 'border-border-base'"
                                        placeholder="••••••••" />
                                    <span
                                        class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 cursor-pointer text-text-muted hover:text-primary"
                                        @click="showPassword = !showPassword">{{ showPassword ? 'visibility_off' :
                                            'visibility' }}</span>
                                </div>
                                <span v-if="getError('password')" class="text-xs text-red-500 mt-1 block">{{
                                    getError('password') }}</span>
                            </div>
                        </div>
                        <div class="flex items-center gap-2 py-2">
                            <input v-model="form.agreeTerms"
                                class="h-4 w-4 rounded border-border-base text-primary focus:ring-primary bg-bg-surface"
                                id="terms" type="checkbox" />
                            <label class="text-xs text-text-muted" for="terms">
                                أوافق على <a class="text-primary hover:underline" href="#">شروط الخدمة</a> و <a
                                    class="text-primary hover:underline" href="#">سياسة الخصوصية</a>
                            </label>
                        </div>
                        <button :disabled="isLoading"
                            class="w-full rounded-xl bg-primary py-4 text-base font-bold text-white shadow-lg shadow-primary/30 transition-all hover:bg-primary/90 hover:shadow-primary/40 active:scale-[0.98] cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            type="submit">
                            <span v-if="isLoading"
                    ...
                    <div class="mt-8 text-center">
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            لديك حساب بالفعل؟
                            <RouterLink class="mr-1 font-bold text-primary hover:underline" to="/login">تسجيل الدخول
                            </RouterLink>
                        </p>
                    </div>
                </div>
...
const handleRegister = async () => {
    ...
    if (!form.fullName.trim()) {
        showToast('خطأ', 'يرجى إدخال الاسم الكامل.', 'error')
        return
    }
    if (!form.email.trim()) {
    ...
    if (!form.password) {
        showToast('خطأ', 'يرجى إدخال كلمة المرور.', 'error')
        return
    }
    if (!form.agreeTerms) {
        showToast('خطأ', 'يجب الموافقة على شروط الخدمة وسياسة الخصوصية.', 'error')
        return
    }
    ...
    try {
        ...
        if (authStore.isAuthenticated) {
            ...
            const dashboard = form.role === 'instructor' ? '/instructor/dashboard' : '/student/dashboard'
            console.log('[Register] Redirecting to:', dashboard)
            router.replace(dashboard)
        } else {
            console.warn('[Register] ⚠️ Register API succeeded but token was not stored!')
            showToast('خطأ', 'حدث خطأ غير متوقع. حاول مرة أخرى.', 'error')
        }
    } catch (error) {
        console.error('[Register] Error:', error)
        setErrors(error)
        const msg = getGeneralError(error)
        showToast('خطأ في التسجيل', msg, 'error')
    } finally {
